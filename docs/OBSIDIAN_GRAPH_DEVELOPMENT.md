...
1. 이메일을 먼저 안정적인 Markdown 파일로 만든다.
2. Markdown frontmatter에 원본 metadata와 LLM 보강 metadata를 저장한다.
3. Markdown 파일을 source of truth로 둔다.
4. SQLite는 검색과 graph traversal을 위한 index/cache로 사용한다.
5. 처음부터 GraphRAG, Neo4j, embeddings에 의존하지 않는다.
...
Outlook email / .msg / export source
  ...
  -> Cline CLI로 LLM metadata 생성 또는 보강
  -> metadata 검증 및 정규화
  -> vault에 Markdown 저장
  -> SQLite reindex
...
DB는 source of truth가 아니라 조회 성능을 위한 재생성 가능한 index다. DB가 깨지거나 삭제되어도 vault의 Markdown을 다시 읽어 같은 DB를 만들 수 있어야 한다.

개발 원칙:

1. Markdown 파일을 source of truth로 둔다.
2. DB에서 vault 파일을 직접 수정하지 않는다.
3. GitHub에는 실제 vault 데이터, `.msg` 원본, 첨부파일, SQLite DB를 저장하지 않는다.
4. reindex는 idempotent해야 한다.
5. LLM이 만든 metadata는 검증 후 저장한다.
6. 원본에서 확정 가능한 metadata와 LLM 추론 metadata를 분리한다.
7. embeddings, entity extraction, GraphRAG는 MVP 이후 단계로 미룬다.
...
이메일은 Obsidian에서 읽기 좋은 Markdown 파일로 저장한다.
...
### 2. LLM이 보강하는 metadata

LLM은 사람이 검색할 때 도움이 되는 보강 metadata를 만든다.
...
LLM 출력은 그대로 저장하지 않는다. 저장 전 validator가 다음을 확인한다.
...
## Cline CLI를 이용한 LLM Metadata 생성

LLM metadata 생성은 Cline CLI로 처리한다.
...
권장 방식은 Cline에게 YAML을 직접 쓰게 하는 것이 아니라, 정해진 JSON schema만 출력하게 한 뒤 importer가 YAML로 변환하는 것이다.
...
그 이유는 JSON schema 검증이 쉽고, LLM이 frontmatter 전체를 망가뜨릴 위험이 작기 때문이다.
...
### Phase 1: LLM Metadata

- Cline CLI로 tag, summary, 보조 metadata를 생성한다.
- Cline 출력은 JSON schema로 제한한다.
- validator로 검증한 뒤 YAML frontmatter에 병합한다.
...
- 원본 metadata와 LLM metadata가 frontmatter에 분리 저장된다.
...
- DB를 삭제하고 reindex해도 같은 결과를 재생성할 수 있다.

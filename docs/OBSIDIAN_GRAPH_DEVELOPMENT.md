...
original_msg: "90_Attachments/email/abc/original.msg"
attachments:
...
1. FTS5 `trigram` tokenizer를 우선 적용한다.
...
- SQLite 빌드가 `trigram` tokenizer를 지원하면 `chunks_fts`는 trigram으로 생성한다.
- 지원하지 않으면 기본 tokenizer로 fallback하되, `status` 또는 `doctor`에서 fallback 상태를 표시한다.
- fallback 상태에서도 API 계약은 유지되어야 하며, 검색 실패가 아니라 검색 품질 저하로 취급한다.
...
- 기본 FTS 검색과 `bm25` 정렬은 MVP API에 들어갈 수 있다.
- graph-boosted ranking은 별도 graph table이 필요하므로 Phase 3 이후 구현 항목이다.
- 문서의 graph API와 graph schema는 구현 완료가 아니라 다음 품질 단계의 설계 기준이다.
...
9. `attachments`, `original_msg`를 `attachments`에 저장한다.

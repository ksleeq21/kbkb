# End-to-End Workflow: Windows Outlook에서 Linux 검색까지
...
`manifest_path`는 증분 SFTP sync 상태를 저장한다. 첫 sync 이후에는 파일 digest가 바뀐 새 파일과 변경 파일만 업로드한다. sync 실패 시 이전 manifest가 유지되어 다음 실행에서 미완료 파일을 다시 시도할 수 있다.
...
## 8. Windows: 일일 Import Scheduling
...
## 9. Linux: Cline CLI로 Raw Markdown 보강
...
현재 validator는 다음을 거부한다.

- `source_id`, `message_id`, `conversation_id`, `received`, `from`, `to`, `cc` 같은 원본 metadata 수정
- YAML/JSON 문법 오류
- 과도하게 많은 tag
- 허용되지 않은 metadata key
- 허용되지 않은 tag 또는 normalization 불가능한 tag를 거부한다.
- vault 외부 path를 metadata로 주입하려는 시도를 거부한다.
- tag taxonomy와 normalization 규칙을 별도 fixture로 회귀 테스트한다.
...
대형 vault에서는 먼저 raw vault 기준 상대 파일 1개나 폴더 1개만 처리해 Cline output과 enriched Markdown을 검증할 수 있다. 이 경우 결과 파일은 enriched vault 아래 동일한 상대 경로에 생성된다.
...
폴더 단위 실행 중 개별 파일이 실패해도 다음 `.md` 파일 처리는 계속된다. 진행률은 `INFO [1/100] enrich rel=...` 형식으로 stderr에 출력되고, 완료 후 stdout에는 target, elapsed time, 전체 Markdown 수, 성공 수, 실패 수가 영문 summary로 출력된다.
...
- `chunks`: 검색용 본문 조각
- `chunks_fts`: SQLite full-text search index table이다
...
python3 cline_skill_obsidian_kb/scripts/kb_search.py "개발망 회의록" --type email --tag project/project-a --after 2026-01-01 --before 2026-12-31
```
...
python3 cline_skill_obsidian_kb/scripts/kb_context.py "ProjectA SSO 관련 근거 찾아줘" --type email --tag project/project-a
```
...
- `[[wiki links]]`, backlink, tag, folder, person, conversation, attachment를 담는 graph table

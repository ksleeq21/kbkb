...
| `/search` returns 401 | API and client token differ | `kb-api status --config <config>` | Set the same `KB_API_TOKEN` for API process and skill shell |
...
| `database_not_indexed` | Reindex has not run or DB path changed | `kb-api status --config <config>` | Run `kb-api reindex --config <config>` |
...
| Empty search result | Vault not synced or not reindexed | `kb-api status --config <config>` | Confirm Markdown files exist under vault path, then reindex |
...
| Outlook unavailable | `pywin32`, classic Outlook, or session issue | `kb-win-sync validate-config --config <config>` | Install Windows extras, use classic Outlook, run while logged in |
| Outlook folder not found | Wrong `outlook_path` | `kb-win-sync --config <config> --dry-run` | Rebuild path from Outlook folder tree |
...
| Duplicate emails skipped | State file already recorded message key | `kb-win-sync status --config <config>` | This is expected; use `--force` only when intentional |
